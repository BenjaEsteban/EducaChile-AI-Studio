...
type FolderFormMode = "folder" | "subfolder";
type FolderContentItem =
  | { kind: "folder"; id: string; folder: FolderTreeNode; createdAt: string; updatedAt: string; name: string }
  | { kind: "video"; id: string; video: Project; createdAt: string; updatedAt: string; name: string };

function isDefaultFolderName(name: string): boolean {
  return name.trim().toLowerCase() === DEFAULT_FOLDER_NAME.toLowerCase();
}

function flattenFolderTree(nodes: FolderTreeNode[], depth = 0): FolderOption[] {
  const rows: FolderOption[] = [];
  for (const node of sortFolders(nodes, "name")) {
...
function sortFolders(items: FolderTreeNode[], mode: SortMode): FolderTreeNode[] {
  return [...items].sort((a, b) => {
    const aIsDefault = isDefaultFolderName(a.name);
    const bIsDefault = isDefaultFolderName(b.name);
...
export default function DashboardPage() {
  ...
  const [folderFormMode, setFolderFormMode] = useState<FolderFormMode>("folder");
  ...
  const folderParentName = folderParentId ? folderNameById.get(folderParentId) ?? "Carpeta seleccionada" : null;

  const filteredProjects = useMemo(() => {
    const byScope = selectedScope === ALL_SCOPE ? projects : projects.filter((project) => project.folder_id === selectedScope);
    return sortProjects(byScope, sortMode);
  }, [projects, selectedScope, sortMode]);

  const folderContentItems = useMemo(() => {
  ...
  }, [filteredProjects, isFolderScope, sortMode, visibleFolders]);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape" && !isSavingFolder) {
  ...
  }, [isFolderFormOpen, isSavingFolder]);

  async function handleCreateProject(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const trimmedName = projectName.trim();
    if (!trimmedName) {
      setProjectFormError("El nombre del video es obligatorio.");
      return;
    }
    ...
    try {
      ...
      setProjectFormError(err instanceof Error ? err.message : "No se pudo crear el video.");
    } finally {
  ...
  async function handleCreateFolder(event: FormEvent<HTMLFormElement>) {
    ...
    if (!trimmedName) {
      setFolderFormError("El nombre de la carpeta es obligatorio.");
      return;
    }
    ...
    try {
      ...
      setFolderFormError(err instanceof Error ? err.message : "No se pudo crear la carpeta.");
    } finally {
      setIsSavingFolder(false);
    }
  }

  function openFolderForm(parentId: string | null, mode: FolderFormMode) {
    setFolderFormMode(mode);
  ...
  function openSubfolderForm(parentFolderId: string) {
    openFolderForm(parentFolderId, "subfolder");
  }
  ...
  async function handleDeleteTarget() {
    ...
    if (deleteTarget.kind === "project") {
      try {
        ...
        setNotice(`Video "${deleteTarget.name}" eliminado.`);
        setDeleteTarget(null);
        await loadDashboard();
      } catch (err) {
        setDeleteDialogError(err instanceof Error ? err.message : "No se pudo eliminar el video.");
      } finally {
    ...
    try {
      ...
      if (isNotEmpty && !deleteNeedsCascade) {
        ...
        setDeleteDialogError("La carpeta contiene subcarpetas o videos. Confirma nuevamente para eliminar todo su contenido.");
      } else {
  ...
  async function handleMoveProject(projectId: string, nextFolderId: string | null) {
    ...
    try {
      ...
      setNotice("Video movido correctamente.");
      await loadDashboard();
    } catch (err) {
      setError(err instanceof Error ? err.message : "No se pudo mover el video.");
    } finally {
  ...
  function renderSidebarTree(nodes: FolderTreeNode[], depth = 0): JSX.Element[] {
    return sortFolders(nodes, "name").flatMap((node) => {
      ...
      const row = (
        <li key={node.id}>
          <div
            ...
            className={`flex items-center gap-1 rounded-md pr-1 ${isSelected ? "bg-brand-50 text-brand-800" : "text-gray-700 hover:bg-gray-50"
          ...
          >
            ...
            <button
              ...
              className={`flex min-w-0 flex-1 items-center gap-2 rounded-md px-2 py-1.5 text-left text-sm ${isSelected ? "font-medium text-brand-800" : "text-gray-700"
                }`}
            >
    ...
    <AppShell title="">
        ...
        <section className="flex h-full min-h-0 flex-col rounded-lg border border-gray-200 bg-white shadow-sm">
          ...
          <div className="flex min-h-0 flex-1 flex-col gap-3 p-4">
            ...
            <button
              ...
              className={`w-full rounded-md px-3 py-2 text-left text-sm ${selectedScope === ALL_SCOPE ? "bg-brand-50 font-medium text-brand-800" : "text-gray-700 hover:bg-gray-50"
                }`}
            >
              Todas las carpetas
            </button>
          ...
          <div className="border-b border-gray-200 px-5 py-4">
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="rounded-full bg-gray-100 px-2.5 py-1 text-xs font-medium text-gray-700">{filteredProjects.length} videos</span>
              </div>
            ...
            {isFolderScope ? (
              <div>
                <div className="mb-3 flex items-center justify-between">
                  <h4 className="text-sm font-semibold text-gray-900">Contenido de esta carpeta</h4>
                  <span className="text-xs text-gray-500">{folderContentItems.length}</span>
                </div>
                {isLoading ? (
                  ...
                  <div className="rounded-lg border border-dashed border-gray-200 px-4 py-6 text-sm text-gray-500">
                    Esta carpeta está vacía. Crea una subcarpeta o agrega un video.
                  </div>
                ) : viewMode === "grid" ? (
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
                    {folderContentItems.map((item) => {
                      if (item.kind === "folder") {
                        ...
                        return (
                          <article
                            ...
                            className={`rounded-lg border p-4 transition ${selectedScope === folder.id
                          ...
                          >
                            <div className="flex items-start justify-between gap-2">
                              <div className="min-w-0 text-left">
                                ...
                                <p className="mt-1 text-xs text-gray-500">{folderProjects.length} videos</p>
                              </div>
                      ...
                      return (
                        <article
                          ...
                          className={`rounded-lg border border-gray-200 bg-white p-4 transition hover:border-brand-200 hover:shadow-sm ${draggingProjectId === project.id ? "opacity-70 ring-2 ring-brand-200" : ""}`}
                        >
                          <div className="flex min-w-0 items-start justify-between gap-3">
                            ...
                            <div className="relative shrink-0 self-start" data-project-menu-root="true">
                              <button
                                ...
                                aria-label={`Acciones para video ${project.name}`}
                              >
                                &#8942;
                              </button>
                  ...
                  <div className="overflow-hidden rounded-lg border border-gray-200">
                    <ul className="divide-y divide-gray-200">
                      {folderContentItems.map((item) => {
                        if (item.kind === "folder") {
                          ...
                          return (
                            <li
                              ...
                              className={`flex cursor-pointer items-center justify-between gap-3 bg-white px-4 py-3 hover:bg-gray-50 ${isDropTarget ? "ring-2 ring-inset ring-brand-300 bg-brand-50/70" : ""}`}
                            >
                              <div className="min-w-0 text-left">
                                <p className="truncate text-sm font-medium text-gray-900">📁 {folder.name}</p>
                                <p className="mt-0.5 text-xs text-gray-500">{folderProjects.length} videos</p>
                              </div>
                              <div className="relative shrink-0" data-folder-menu-root="true">
                                <button
                                  type="button"
                        ...
                        return (
                          <li
                            key={`video-row-${project.id}`}
                            draggable
                            onDragStart={() => handleProjectDragStart(project.id)}
                            onDragEnd={handleProjectDragEnd}
                            className={`flex items-center justify-between gap-3 bg-white px-4 py-3 hover:bg-gray-50 ${draggingProjectId === project.id ? "opacity-70 ring-2 ring-inset ring-brand-200" : ""}`}
                          >
                            <div className="min-w-0 text-left">
                              <Link href={`/projects/${project.id}`} className="block">
                                <p className="truncate text-sm font-medium text-gray-900 hover:text-brand-700">{project.name}</p>
                              </Link>
                              <p className="mt-0.5 truncate text-xs text-gray-500">{project.description || "Sin descripción"}</p>
                            </div>
                            <div className="relative shrink-0" data-project-menu-root="true">
                              <button
                                type="button"
                                onClick={() => setOpenProjectMenuId((prev) => (prev === project.id ? null : project.id))}
                                className="inline-flex h-8 w-8 items-center justify-center rounded-md border border-gray-300 text-sm text-gray-600 hover:bg-gray-100"
      ...
      {isFolderFormOpen ? (
        <div
          ...
          onClick={(event) => {
            if (event.target === event.currentTarget && !isSavingFolder) {
              setIsFolderFormOpen(false);
              setFolderFormError(null);
            }
          }}
        >
          <div className="w-full max-w-md rounded-lg border border-gray-200 bg-white p-5 shadow-xl">
            <h4 className="text-base font-semibold text-gray-900">
              {folderFormMode === "subfolder" ? "Crear subcarpeta" : "Crear carpeta"}
            </h4>
            <p className="mt-2 text-sm text-gray-600">
              {folderFormMode === "subfolder"
                ? "Ingresa el nombre de la nueva subcarpeta."
            ...
            {folderParentName ? (
              <div className="mt-3 rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-700">
                <span className="font-medium">Carpeta padre:</span> {folderParentName}
              </div>
            ) : null}
            ...
            <form onSubmit={handleCreateFolder} className="mt-4 space-y-4">
              <label className="block">
                <span className="text-sm font-medium text-gray-700">
                  {folderFormMode === "subfolder" ? "Nombre de la subcarpeta" : "Nombre de la carpeta"}
                </span>
                <input
                  value={folderName}
                  onChange={(event) => {
                    setFolderName(event.target.value);
                    if (folderFormError) setFolderFormError(null);
                  }}
                  ...
                  placeholder={folderFormMode === "subfolder" ? "Unidad 1" : "Nueva carpeta"}
                  maxLength={255}
                  autoFocus
                />
              </label>
              ...
              {folderFormError ? (
                <p className="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700">{folderFormError}</p>
              ) : null}
              ...
              <div className="flex items-center justify-end gap-2">
                <button
                  ...
                  className="rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:cursor-not-allowed disabled:bg-gray-100"
                >
                  Cancelar
                </button>
                <button
      ...
      {deleteTarget ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 p-4">
          <div className="w-full max-w-md rounded-lg border border-red-200 bg-white p-5 shadow-xl">
            <h4 className="text-base font-semibold text-gray-900">
              {deleteTarget.kind === "folder" ? "Eliminar carpeta" : "Eliminar video"}
            </h4>
            <p className="mt-2 text-sm text-gray-600">
            ...
            <p className="mt-1 text-sm text-red-700">
              {deleteTarget.kind === "folder"
                ? "Si contiene subcarpetas o videos, será necesario confirmar la eliminación total."
                : "También se eliminarán presentaciones, assets y configuraciones asociadas al video."}
            </p>
      ...
      {isProjectFormOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/40 p-4">
          <div className="w-full max-w-lg rounded-lg border border-gray-200 bg-white p-5 shadow-xl">
            <h4 className="text-base font-semibold text-gray-900">
              {projectFolderId ? "Crear video en esta carpeta" : "Crear video"}
            </h4>
            <p className="mt-2 text-sm text-gray-600">
              Completa la información del video y confirma su creación.
            </p>

            <form onSubmit={handleCreateProject} className="mt-4 space-y-4">
              <div className="rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-700">
                <span className="font-medium">Ubicación:</span>{" "}
                {projectFolderId ? folderNameById.get(projectFolderId) ?? "Carpeta seleccionada" : DEFAULT_FOLDER_NAME}
              </div>

              <label className="block">
                <span className="text-sm font-medium text-gray-700">Nombre del video</span>
                <input
                  value={projectName}
                  onChange={(event) => {
              ...
              {!projectFolderId ? (
                <label className="block">
                  ...
                  <select
                    ...
                    className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100"
                  >
                    <option value="">{DEFAULT_FOLDER_NAME} (predeterminada)</option>
                    {folderOptions.map((folder) => (
                      <option key={folder.id} value={folder.id}>
                        {"\u00A0".repeat(folder.depth * 2)}
                        {folder.name}
              ...
              {projectFormError ? (
                <p className="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700">{projectFormError}</p>
              ) : null}
